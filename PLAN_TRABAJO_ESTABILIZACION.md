...
Se agregan `Dockerfile` y `.dockerignore` para contenerizar la API FastAPI.
...
| Diagnostico DB protegido | Si | Se agrega `/diagnostics/db-metrics`, protegido con `X-API-Key`, de solo lectura y sin filas de negocio. Devuelve metadatos seguros de conexion, conteos por anio, conteos por metrica y presencia de metricas esperadas para validar que Cloud Run/Supabase vean el mismo dataset que local. En PostgreSQL se consulta explicitamente `public.infonavit_historico` para evitar diferencias por `search_path`. | Desplegar nueva revision Cloud Run y validar que el endpoint deje de responder `404`; despues validar que el reporte extendido productivo ya no devuelva ceros. |
